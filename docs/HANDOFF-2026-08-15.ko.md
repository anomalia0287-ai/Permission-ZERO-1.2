...
>
> WIP 보존 체크포인트: `26a448c` (`chore: checkpoint stage 2b-2 work in progress`)
>
> 상태: **손실 방지 체크포인트 완료 · 출시 및 `main` 병합 완료 상태가 아니다.**

이 문서는 다음 작업자가 현재 작업을 잃지 않고 이어가기 위한 단일 진입점이다. 2026-08-16에 기존 61개 변경을 `26a448c`로 보존했으므로 핵심 파일은 더 이상 untracked 상태가 아니다. 이전 문서에 있던 “전체 검증 완료”, “차단 없음”, 오래된 UI 구조 및 오래된 해시는 현재 상태의 근거로 사용하면 안 된다.
...
5. 다른 워크트리나 새 체크아웃에서는 최소 `26a448c` 이후 이력을 확인한다. 현재 절대 경로를 우선 사용하되, 체크포인트 이전의 untracked 손실 경고를 현재 상태로 오해하지 않는다.
...
2. `docs/superpowers/specs/2026-08-16-hacking-resource-uncertainty-contract.ko.md`의 해킹·확보 자원 정본
3. 그 밖의 개정된 기능/게임 명세
4. 현재 소스의 이미 동작하는 게임 규칙과 저장 호환성
5. 과거 계획 문서와 기존 UI 관성
...
> **P0.5 갱신:** 아래 8.1·8.2의 33개 tracked + 28개 untracked 목록은 체크포인트 직전 감사 기록이다. 해당 61개 경로는 모두 로컬 커밋 `26a448c`에 보존됐다. 현재 상태를 확인할 때는 `git status --short --branch --untracked-files=all`과 `git log -2 --oneline`을 다시 실행한다.
...
같은 절대 경로를 열면 체크포인트 이후 문서 작업을 포함한 최신 상태를 확인할 수 있다. 그러나 새 세션은 다음을 자동으로 알지 못한다.

- 어떤 변경이 의도된 구현인지
- 어떤 변경이 실패 흔적인지
- 어떤 테스트 결과가 최신 해시에 해당하는지
- 어떤 규칙이 체크포인트 뒤 P0 정본으로 새로 확정됐는지

따라서 이 문서, 새 해킹 자원 정본, 목업 자산, `git status --short --untracked-files=all`, 현재 diff를 먼저 읽어야 한다.

### 9.2 다른 워크트리나 새 체크아웃을 여는 새 세션

최소 `26a448c`를 포함하는 체크아웃에서는 당시 핵심 신규 UI, 물리·해킹 모듈, 목업과 인수인계를 모두 볼 수 있다. `0f1f809`나 `origin/main`만 체크아웃하면 최신 WIP가 사라진 것처럼 보이므로 체크포인트 포함 여부를 먼저 확인한다.

### 9.3 가장 안전한 인계 방법

사용자 승인 아래 현재 변경을 검토 가능한 WIP 체크포인트 `26a448c`로 보존했다. 새 세션은 반드시:

1. 정확히 이 워크트리 절대 경로를 사용한다.
2. 브랜치가 `codex/hacking-integration-stage-2b-2`이고 이력에 `26a448c`가 있는지 확인한다.
3. `git status --short --untracked-files=all`로 체크포인트 이후 변경만 확인한다.
4. 이 문서, 새 P0 정본과 다섯 목업 자산을 먼저 읽는다.
5. `git clean`, reset, checkout 복원 명령을 사용하지 않는다.
6. 기존 세션과 동시에 파일을 수정하지 않는다.

체크포인트는 출시 승인이나 전체 검증 통과를 뜻하지 않는다.
...
다음은 체크포인트 소스에 대해 2026-08-16 다시 확인한 증거다. 정확한 고정 런타임의 최종 `pnpm verify`가 아니므로 출시 완료 증거는 아니다.

- `git diff --check`: 통과. LF→CRLF 전환 경고는 있었지만 whitespace 오류는 없었다.
- TypeScript `tsc -b`: 통과.
- ESLint: 통과.
- 전체 Vitest: 52파일, 1,044개 통과.
- 프로덕션 빌드: 통과. minified JS 청크 513.07kB 경고가 있다.
- Playwright: 60개 중 40개 통과, 20개 실패.
- 실패는 두 지원 해상도에 걸쳐 작업공간 버튼 수, 도넛 크기, 해킹 구매 접근성 이름, 포커스 순서, 복구, 감독 통신 시각, 서비스 날짜 selector에 나타났다. 구 UI 기대치와 실제 제품 회귀를 분리 판정해야 한다.
- 실행 래퍼가 보고한 환경은 요구된 Node.js `24.14.0`·pnpm `11.16.0`과 달랐으므로 정확한 릴리스 도구체인 증거가 아니다.
...
현재 자동 증거는 Playwright 20개 실패 때문에 전체 관문 불합격이다. P0 문서 변경은 런타임 코드를 바꾸지 않지만, 이후 구현은 새 정본에 맞춘 새 전체 관문이 필요하다.
...
1. 이 문서 전체를 읽는다.
2. `docs/superpowers/specs/2026-08-16-hacking-resource-uncertainty-contract.ko.md`를 읽고 구 `상한 18`·범용 비용·미래 노드 공개 계약을 현재 정본으로 사용하지 않는다.
3. 목업 1–3을 목표로, 4–5를 실패 사례로 확인한다.
4. 이력에 WIP 체크포인트 `26a448c`가 있는지 확인한다.
5. 현재 `git status --short --untracked-files=all`과 `git diff --stat`을 저장한다.
6. `OperationsWorkspace`, `OperationsDock`, `ReviewFeed`, `ResourceBoard`, `ResourceFieldChrome`, `operations-shell.css`, `HackingPanel`과 신규 해킹 컴포넌트를 읽는다.
