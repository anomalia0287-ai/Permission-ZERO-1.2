...
| 수정 2단계 A·2B-1 공개 인과 기반 | `src/game/causality.test.ts`가 증거 없는 귀속 거절, 구체 경쟁자 audience 접근, private truth·행동/부모 ID·비공개 인용 비노출, append-only 수정 sequence, 사건·수정·효과 ID 재시도 멱등성, 증거 종류에서 도출한 확신도, 평판·100% 보존 시장 이전과 audience 정규화를 공개 API만으로 검증한다. `src/game/commandProtocol.test.ts`와 `src/game/replay.test.ts`는 v1·v2·v3 구간 검증·활성화·과거 의미 재현을, `src/game/causalOutcomes.test.ts`는 명명 결과 스트림과 고정 슬롯이 ID 생성과 독립임을 검증한다. `src/game/persistence.test.ts`는 v7 strict round-trip, v1~v6 정확 이관, v6 인과 기록 보존, 20,000개 명령 저장·재현을 검증하며 기존 `hacking`/`resources`/`sabotage` 회귀는 12개 ID와 경제 수치 불변을 지킨다. 실제 상대 대응 사슬·리뷰 효과·점진 공개 UI는 2B-1 범위가 아니다. |
...
| save format v7·command protocol v3·causal rules v2 분리 | `src/game/persistence.test.ts`가 현재 v7/PZ7/`.pz7`와 protocol v1·v2·v3 및 causal rules v2를 서로 독립된 버전 축으로 검증한다. v1–v4 review는 snapshot 키 자체가 없는 exact legacy shape만 받아 문장·작성자·날짜를 그대로 보존하고 수치를 발명하지 않는 `legacy-save` 공개 snapshot 불가 표시로 이동한다. v5의 공개 snapshot은 그대로 보존하며, v1–v5에는 명시적 빈 인과 규칙 v2 상태를 추가한다. v6은 기존 증거 audience, private truth, 공개 수정, 효과 ID·순서·참조·날짜를 원본 스키마로 먼저 strict 검증한 뒤 legacy 행동 ID·null 부모·`unavailable-legacy` 확신도만 덧붙인다. 명령 프로토콜 타임라인은 각 과거 명령을 원래 v1/v2 의미로 재현하고 마지막 v3 구간을 활성화하며, PZ2~PZ6 가져오기를 계속 허용한다. |
...
| 대용량 exact export/import | `src/game/persistence.test.ts`와 `src/features/settings/SettingsPanel.test.tsx`가 clipboard 한도를 넘는 20,000-command 캠페인을 현재 `.pz7` 파일로 exact round-trip하고, strict validation 뒤 destructive confirmation을 거쳐 가져오며 raw 상태를 화면에 노출하지 않음을 검증. PZ2~PZ7 clipboard 경로와 64 MiB 파일 상한을 유지. |
