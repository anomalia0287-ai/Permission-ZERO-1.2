...
| save format v4와 command protocol v1/v2 분리 | `src/game/persistence.test.ts`가 v4 envelope에서 protocol v1과 v2를 모두 명시적으로 수용하고 journal을 한 번만 저장함을 검증한다. v4 checkpoint/chunk integrity hash는 schema와 별개로 유효한 JSON의 변경도 거부한다. 실제 v1 fixture, legacy transfer prefix, exact-key v2/v3 envelope, PZ2/PZ3 clipboard는 손실 없이 v4 runtime journal로 이동하며 terminal 상태도 정규화된다. |
...
| 대용량 exact export/import | `src/game/persistence.test.ts`와 `src/features/settings/SettingsPanel.test.tsx`가 clipboard 한도를 넘는 20,000-command 캠페인을 `.pz4` 파일로 exact round-trip하고, strict validation 뒤 destructive confirmation을 거쳐 가져오며 raw 상태를 화면에 노출하지 않음을 검증한다. 일반 PZ2/PZ3/PZ4 clipboard 경로도 유지된다. 파일은 별도 64 MiB 상한을 적용하고 초과 파일은 내용을 메모리에 읽기 전에 거부한다. |
...
- 브라우저가 읽는 자료는 실제 v1/v2/v3/v4 저장 decoder를 통과한다. PZ2/PZ3/PZ4는 실제 설정 UI의 검증·확인 경로를 사용한다.
