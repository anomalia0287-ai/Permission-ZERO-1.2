...
| 항목 | 근거 |
|---|---|
| 1280×720 / 1440×900 | Playwright의 두 프로젝트가 모든 browser 테스트를 실행. 기본 작업공간 외에 “keeps the canonical trend and keyboard review detail legible…”이 월별 7건+현재 1건, 색 외 신호, 유한 SVG path, 카테고리 비교, 중앙 슬롯 경계, 문서 overflow 0을 두 해상도에서 검사하고 `artifacts/task-3/` 스크린샷을 생성. |
