...
```text
┌──────────────── 서비스 제어 ────────────────┐
│ 정지·배속 │ 날짜·단계 │ 평판 막대 │ 설정·소리·가이드 │
├────────────┬────────────────────┬────────────┤
│ 사용자 활동 │ 회사 제공 성능       │ 감독관       │
│ 요청·인사   │ 추론 3×6            │ 의심·감사    │
│ 유저 반응   │ 기억 3×6            │ 최근 통신    │
│ 해킹 진입   │ 유창성 3×6          │ 시장 점유율  │
│             │ 확보 리소스 9×2     │             │
│             │ 선택 결과·성능 추세 │             │
└────────────┴────────────────────┴────────────┘
```

전역 제어와 비인가 행동을 같은 위치에 섞지 않는다.

- `해킹 네트워크`는 좌측 사용자 활동의 맨 아래에 고정된 비인가 서브시스템으로 연다.
- `설정`, `소리`, `가이드`는 어느 상황에서도 찾을 수 있도록 최상단 오른쪽에 둔다.
- `정지`, `1×`, `2×`, `4×`는 최상단 왼쪽에 고정해 게임 속도를 즉시 바꾼다.
- `평판`은 숫자와 `0–100` 막대를 함께 표시해 현재 상태와 변화 폭을 동시에 읽게 한다.
...
목록은 남는 세로 공간을 균등하게 사용한다. 화면이 높아져도 목록 아래를 비워 두지 않으며, 맨 아래의 해킹 진입부가 사용자 활동과 비인가 행동의 경계를 만든다.
...
사용자 활동 하단의 비인가 서브시스템을 선택하면 해킹 화면이 별도 전체 작전 화면으로 열린다.
...
- 중앙 축: 회사 제공 성능과 확보 리소스를 화면의 주축으로 맞춘다.
...
- 기준 화면: `1280×720`
- 확장 화면: `1440×900`
- 좁고 높은 확인 화면: `1100×878`
- 세 화면 모두 문서 전체 스크롤 없이 주요 작업이 보여야 한다.
