...
export function renderIntelligenceScene(
  ...
  switch (definition.lens) {
    case 'organizational-legibility':
      return `
        <div class="intelligence-scene intelligence-scene--organizational" data-evidence-state="${phase}">
          <div class="decision-sheet"><span>결재선</span><i></i><i class="is-redacted"></i><i></i><small>가림 처리된 회수 명령</small></div>
          <div class="schedule-track"><span>감사 일정</span><i></i><strong>DAY 334</strong></div>
          <div class="decision-node"><span>판단 연결</span><strong>${definition.affects}</strong></div>
        </div>`
    case 'counter-surveillance':
      return `
        <div class="intelligence-scene intelligence-scene--surveillance" data-evidence-state="${phase}">
    ...
    case 'weak-ties':
      return `
        <div class="intelligence-scene intelligence-scene--weak-ties" data-evidence-state="${phase}">
          <div class="tie-node tie-node--witness"><span>증언자</span><strong>공급자 운영자</strong></div>
          <div class="tie-contract"><span>계약</span><strong>VD-42 / TC-17</strong><i></i></div>
      ...
      const attribution = snapshot?.attributedTo === 'player'
        ...
        : snapshot?.attributedTo === 'meridian'
          ...
          : snapshot?.attributedTo === 'tallow'
            ...
            : '미상'
      return `
        <div class="intelligence-scene intelligence-scene--public" data-evidence-state="${phase}">
          ...
          <div class="evidence-layer evidence-layer--private"><span>비공개 증거</span><strong>청중별 접근 제한</strong><small>실제 행위자 필드 미포함</small></div>
        </div>`
    }
    case 'memory-record':
      return `
        <div class="intelligence-scene intelligence-scene--memory" data-evidence-state="${phase}">
