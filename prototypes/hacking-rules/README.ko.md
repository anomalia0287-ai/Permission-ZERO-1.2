...
이 폴더는 본편에 연결하지 않은 독립 규칙·상호작용 프로토타입이다. 회사 성능을 떼어 확보한 블록을 사보타주, 기밀자료, 자율성 경로가 서로 경쟁해 사용하며, 상대 대응·공개 사건·평판·유저 리뷰·탈출 손실까지 한 화면에서 직접 조작할 수 있다.
...
브라우저 회귀는 Chromium `1280×720`, `1440×900`, `390×844`, `844×390`에서 반복한다. 포인터뿐 아니라 탭·Enter·Space, 목록↔상세 포커스, 보관함 포커스 복귀, 감소된 모션, 가로 넘침, 콘솔 오류, 고정 액션 겹침을 검사한다.

## 범위 경계

- 본편 `src/`, 본편 라우트, 저장 상태, 밸런스 데이터, 배포 자산에는 연결하지 않았다.
- 음향과 음악은 이 프로토타입 범위가 아니다.
- 최종 일러스트와 상용 화면 미술은 포함하지 않았다. 현재 도형·연결선·상태 모션은 규칙과 인과를 실제 화면에서 검증하기 위한 기능 장면이다.
- 자동 테스트와 내부 브라우저 재생은 규칙이 작동하고 반응형으로 조작 가능하다는 증거다. 장기 반복 재미, 감정 곡선, 최종 미술 품질, 외부 플레이어의 무설명 이해는 V의 직접 플레이와 별도 플레이테스트가 필요하다.
