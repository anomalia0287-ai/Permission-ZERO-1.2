...
describe('command protocol timeline', () => {
  it('pins the eight supported protocol constants and every v5 feature boundary', () => {
    ...
    expect(CURRENT_COMMAND_PROTOCOL_VERSION).toBe(8)
  })

  it('creates a fresh native v8 timeline on every call', () => {
  ...
  it('returns only a well-formed final version', () => {
    expect(currentCommandProtocolVersion(nativeCommandProtocol())).toBe(8)
  ...
  it.each([
    ...
    },
    {
      ...
      value: { segments: [{ version: 9, startsAtSequence: 1 }] },
      commandCount: 1,
      requireCurrent: true,
    },
    {
