...
function nativeV2Protocol(commandCount: number): CommandProtocolMetadata {
  return {
    segments: [
      ...
      { version: 8, startsAtSequence: commandCount + 1 },
    ],
  }
}
...
function historicalQualityReplayFixture(
  ...
  const commandProtocol: CommandProtocolMetadata = {
    segments: [
      ...
      { version: 8, startsAtSequence: commands.length + 1 },
    ],
  }
  return {
    seed,
...
describe('deterministic command replay', () => {
  ...
  ])('replays $label provenance independently from the same 8@1 timeline', ({
    label,
    replayBootstrap,
    opening,
    prefixCount,
  ...
  it('replays 1@1, 2@32, and 3@51 under original semantics before activating 8@52', () => {
  ...
  it('activates an empty final v8 segment after replaying the v2 history', () => {
    const commands = [
      { type: 'SET_SPEED', speed: 1 },
