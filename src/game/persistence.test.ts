...
function task5ScheduledQualitySabotage(seed: string): CampaignState {
  ...
  const purchaseBlockIds = state.resources.reserve.filter((blockId) => {
    if (!blockId) return false
    const origin = state.resources.blocks[blockId].origin
    return origin === 'fluency' || origin === 'reasoning'
  }).filter((blockId, index, blockIds) => {
    const origin = state.resources.blocks[blockId as string].origin
    return origin === 'fluency' || blockIds.slice(0, index).every(
      (prior) => state.resources.blocks[prior as string].origin !== 'reasoning',
    )
  }) as string[]
  if (purchaseBlockIds.length !== 3) {
    throw new Error('Task 5 quality fixture requires an exact 1/0/2 vector')
  }
...
describe('versioned campaign saves', () => {
  ...
  it('migrates a valid v10 intrusion checkpoint and promotes future commands to protocol 8', () => {
    ...
    expect(decoded.envelope.commandProtocol).toEqual({
      segments: [
        ...
        { version: 8, startsAtSequence: legacy.commandSequence + 1 },
      ],
    })
  })
  ...
  it('encodes a canonical v11 envelope without duplicating runtime-only fields', () => {
    ...
    expect(raw).toMatchObject({
      version: 11,
      commandProtocol: {
        segments: [{ version: 8, startsAtSequence: 1 }],
      },
      replayBootstrap: {
        openingVersion: 2,
  ...
  it.each([
    ...
    [2, 0, 0, '8@1'],
    [2, 19, 0, '2@1;8@20'],
    [2, 50, 31, '1@1;2@32;8@51'],
    [3, 50, 31, '1@1;2@32;8@51'],
    [4, 50, 31, '1@1;2@32;8@51'],
    [5, 50, 31, '1@1;2@32;8@51'],
    [6, 50, 31, '1@1;2@32;8@51'],
  ] as const)(
  ...
  it('keeps zero-command v1 and v2 provenance distinct after both migrate to 8@1', () => {
  ...
  it.each(TASK_5_ROLLBACK_PROFILES)(
    ...
    ({ actionId, opportunityDays }) => {
      ...
      expect(scheduled.commandSequence).toBe(11)
      ...
      expect(uninterrupted.commandSequence).toBe(12)
  ...
  it('uses v6 final-choice semantics for the next purchase after a v5 save promotion', () => {
    ...
    const stageNineVector = autonomyCostVectorForStage(
      ...
      AUTONOMY_STAGE_TOTALS_V7.length,
      AUTONOMY_STAGE_TOTALS_V7[AUTONOMY_STAGE_TOTALS_V7.length - 1],
    )
