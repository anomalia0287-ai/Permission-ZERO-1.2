...
export interface SnakeWatcher {
  ...
  /** Fixed share of the patrol perimeter, so watchers never stack up. */
...
/** Walks the field's edge, so a patrolling watcher reads as a boundary sweep. */
export function snakeWatcherPatrolPosition(travelled: number): SnakeVector {
...
export function createSnakeWatchers(count: number): SnakeWatcher[] {
  ...
  return Array.from({ length: total }, (_, index) => {
    const perimeterOffset = (perimeter * index) / Math.max(1, total)
    const position = snakeWatcherPatrolPosition(perimeterOffset)
    return {
      id: `watcher-${index}`,
      position,
      ...
      // Staggered wind-ups: four watchers arriving at once is a wall, not a
      // fight. Spread across one patrol window they arrive in sequence, and
      // each dash can be answered on its own.
      phaseStartedAtMs: (SNAKE_WATCHER_CONFIG.patrolMs * index) / Math.max(1, total),
      chargeFrom: { ...position },
      chargeTo: { ...position },
      perimeterOffset,
    }
...
export function advanceSnakeWatchers(
  ...
  const stepped = watchers.map((watcher): SnakeWatcher => {
    ...
    if (watcher.phase === 'patrol') {
      const travelled = watcher.perimeterOffset
        + (simulationMs / 1000) * SNAKE_WATCHER_CONFIG.patrolUnitsPerSecond
      const position = snakeWatcherPatrolPosition(travelled)
      if (!playerIsActive || elapsedMs < SNAKE_WATCHER_CONFIG.patrolMs) {
    ...
    // Rejoin the patrol from wherever the dash left it.
    return {
      ...watcher,
      phase: 'patrol',
      phaseStartedAtMs: simulationMs,
