...
function ProgressImportControl({
  ...
  return (
    <section className="progress-import" aria-label="진행 가져오기">
      <div className="progress-file-actions">
        ...
        <label>
          ...
          <input
            ...
            accept=".pz6,.pz5,.pz4,.pz3,.pz2,application/vnd.permission-zero.progress+json"
            onChange={(event) => {
      ...
      <p>클립보드는 최대 1 MiB의 <code>PZ2:</code>/<code>PZ3:</code>/<code>PZ4:</code>/<code>PZ5:</code>/<code>PZ6:</code> 자료를 지원합니다. 더 큰 진행은 최대 64 MiB의 .pz6 파일로 검증하고 복원할 수 있습니다.</p>
      <button
        ...
        >
          ...
          {copyState === 'export-too-large' ? (
            <>
              ...
              <p>.pz6 진행 파일로 전체 상태와 기록을 정확히 다운로드할 수 있습니다.</p>
              <p>브라우저 저장 공간은 유한하므로 경고가 계속되면 파일을 안전한 곳에 보관하세요.</p>
            </>
          ) : (
            <>
              <p>현재 시드 <code>{state.campaignSeed}</code>를 복사하거나 진행 내보내기를 복사해 수동으로 보관하세요.</p>
              <p>보관한 <code>PZ2:</code>/<code>PZ3:</code>/<code>PZ4:</code>/<code>PZ5:</code>/<code>PZ6:</code> 자료는 설정의 ‘진행 가져오기’에서 검증하고 복원할 수 있습니다.</p>
            </>
          )}
          <div>
